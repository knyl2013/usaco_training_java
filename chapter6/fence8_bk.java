...
public class fence8 {
	...
	static List<Integer> changeHelper(int val, boolean[] railMarked)
	{
		...
		for (int i = 0; i < k; i++)
			if (!railMarked[i])
				canUse.add(i);
        if (canUse.isEmpty()) return new ArrayList<>();
        // System.out.println("canUse: " + canUse);
		int len = canUse.size();
		memo = new int[val+1][len];
  ...
  //       }
        // watch("dfs(val, len-1)", dfs(val, len-1));
        // watch("dfs(val-rails[canUse.get(len-1)], len-1)", dfs(val-rails[canUse.get(len-1)], len-1));
        // watch("dfs(val, len-2)", dfs(val, len-2));
        // watch("dfs(val, len-3)", dfs(val, len-3));
        
		int idx = len-1;
		int v = val;
		while (idx >= 0 && v > 0) {
			int ignoreIdx = dfs(v, idx-1);
			if (v >= rails[canUse.get(idx)] && dfs(v-rails[canUse.get(idx)], idx-1) <= ignoreIdx) {
				ans.add(canUse.get(idx));
	...
	static void solve()
    {
		...
		for (int i = 0; i < m; i++)
			rails[i] = ni();
		int ans = brute(boards, rails, n, m);
		out.printf("%d\n", ans);
    }
...
static class MaxFlow {
    ...
    private static void watch(String name, Object x)
    {
