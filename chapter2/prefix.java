...
public class prefix {
    static void solve()
    {
        ...
        try {
            while (!(cur = ns()).isEmpty()) sb.append(cur);
        } catch(Exception e) {};
        ...
        for (String p : ps) {
            for (int i = 0; i < n; i++) {
                boolean good = true;
                for (int j = 0; j < p.length() && i + j < n; j++) {
                ...
                if (good) {
                    adjList.get(i).add(p.length());
                }
            }
        }

        // keep expanding the rightmost until finish
        boolean[] reachable = new boolean[n+1];
        reachable[0] = !adjList.get(0).isEmpty();
        int rightmost = 0;
        for (int i = 0; i < n; i++) {
            ...
            for (int next : adjList.get(i)) {
                if (i+next <= n) {
                    ...
                    rightmost = Math.max(rightmost, i+next);
                }
            }
        }
        out.printf("%d\n", rightmost);
